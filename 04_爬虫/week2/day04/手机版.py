...
def get_one_page():
    ...
    # browser.find_element_by_id('kw').send_keys('淘宝')
    ...
    # browser.find_element_by_id('q').send_keys(keys.Keys.CONTROL + keys.Keys.SHIFT + 'i')

    # Actions action = new Actions(browser)

    # ActionChains(browser).key_down(Keys.CONTROL).key_down(Keys.SHIFT).send_keys('i').perform()
    ...
    # browser.find_element_by_id('q').send_keys('白酒')
    ...
    for i in range(98):
        ...
        time.sleep(2)
        # 隐藏遮罩层
        tstart = browser.find_element_by_id('tstart')
        tstart.is_displayed()

        # 执行jq代码
        # browser.execute_script("$('.item next').click()")

        browser.find_element_by_xpath('//li[@class="item next"]').click()
        # browser.find_element_by_css_selector('a[trace="srp_bottom_pagedown"]').click()
        time.sleep(5)
...
def get_data(ws, con):
    tree = etree.HTML(con)
    goods_list = tree.xpath('//div[contains(@class,"item J_MouserOnverReq  ")]')
    print(goods_list)
