...
def get_one_page():
    ...
    ws = wb.active
    ws.append(['商品名', '价格', '付款人数', '店铺', '地址', '链接'])
...
def get_data(ws, con):
    ...
    for goods in goods_list:
        ...
        address = goods.xpath(
            ...
            0]
        lianjie = \
        goods.xpath('.//div[@class="ctx-box J_MouseEneterLeave J_IconMoreNew"]//a[@class="J_ClickStat"]/@href')[0]
        # print(name+' '+price+' '+pnum+' '+shop_name+' '+address)
        # print(name)
        with open('淘宝商品--%s.txt', 'a', encoding='utf-8')as f:
            f.write((name + ' ' + price + ' ' + pnum + ' ' + shop_name + ' ' + address + ' ' + lianjie))
            f.write('\n')
        ws.append([name, price, pnum, shop_name, address, lianjie])
